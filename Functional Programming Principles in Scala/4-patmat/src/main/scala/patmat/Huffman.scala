...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    // Groupby transforms it to a Map[Char, List[Char]]
    // MapValues then maps every list to its size
    // The result is a mapping from Char to the size of the groups
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    val nodes = until(singleton, combine)(trees)
    nodes.head
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    // Subfunction needed so that we can go back to the root after each traversal.
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encode_char(tree: CodeTree)(char: Char): List[Bit] = {
      tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) if chars(left).contains(char) =>
          0 :: encode_char(left)(char)
        case Fork(left, right, _, _) =>
          1 :: encode_char(right)(char)
      }
    }

    text.flatMap(char => encode_char(tree)(char))
  }
